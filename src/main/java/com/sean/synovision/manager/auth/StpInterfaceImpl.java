...
public class StpInterfaceImpl implements StpInterface {
    ...
    public List<String> getPermissionList(Object loginId, String loginType) {
        ...
        // 管理员权限，表示权限校验通过
        List<String> ADMIN_PERMISSIONS = spaceUserAuthManager.getPromissionByRole(SpaceRoleEnum.ADMIN.getValue());
        // 获取上下文对象
        ...
        if (loginUser == null) {
            throw new BussinessException(ErrorCode.NO_AUTH_ERROR, "用户未登录");
        }
        Long userId = loginUser.getId();
        // 优先从上下文中获取 SpaceUser 对象
        SpaceMember spaceUser = authContext.getSpaceMember();
        if (spaceUser != null) {
            //如果 spaceUser 对象不为 null，则直接返回权限
            return spaceUserAuthManager.getPromissionByRole(spaceUser.getSpaceUserRole());
        }
        // 如果有 spaceUserId，必然是团队空间，通过数据库查询 SpaceUser 对象
        Long spaceUserId = authContext.getSpaceUserId();
        if (spaceUserId != null) {
            spaceUser = spaceMemberService.getById(spaceUserId);
            if (spaceUser == null) {
                throw new BussinessException(ErrorCode.NOT_FOUND_ERROR, "未找到空间用户信息");
            }
            // 取出当前登录用户对应的 spaceUser
            SpaceMember loginSpaceUser = spaceMemberService.lambdaQuery()
                    .eq(SpaceMember::getSpaceId, spaceUser.getSpaceId())
            ...
            return spaceUserAuthManager.getPromissionByRole(loginSpaceUser.getSpaceUserRole());
        }
        ...
        if (spaceId == null) {
            ...
            Picture picture = pictureService.lambdaQuery()
                    ...
                    .one();
            if (picture == null) {
                throw new BussinessException(ErrorCode.NOT_FOUND_ERROR, "未找到图片信息");
            }
        ...
        if (space == null) {
            throw new BussinessException(ErrorCode.NOT_FOUND_ERROR, "未找到空间信息");
        }
        // 根据 Space 类型判断权限
        if (space.getSpaceType() == SpaceTypeEnum.PRIVATE.getValue()) {
            ...
            spaceUser = spaceMemberService.lambdaQuery()
    ...
    private SpaceUserAuthContext getAuthContextByRequest() {
        ...
        if (ObjUtil.isNotNull(id)) {
            // 获取到请求路径的业务前缀，/SynoVision/picture/aaa?a=1
